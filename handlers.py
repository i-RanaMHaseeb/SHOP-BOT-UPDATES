...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        try:
            ...
            pass
        # Send The Main Menu Again So User Is Never Left With A Blank Screen
        await context.bot.send_message(
            ...
            reply_markup=kb.main_menu_keyboard(uid),
        )
        return
    if data == "close_msg":
        try:
    ...
    if data == "gf_mail":
        # Clear Any Stale State Before Starting A Fresh Broadcast
        clear_state(uid)
        set_state(uid, "mail_wait_content")
        log.info(f"Broadcast: Admin {uid} Opened Mail Prompt — State Set To mail_wait_content")
        return await safe_edit(
            query,
            "📢 *Send Out A Post To Be Distributed To Users*\n"
            "❗ Posts With Any Media Files Are Supported\n\n"
            "✏️ Send The Message (Text, Photo, Video, Etc.) Now.",
        )
    if data == "mail_send":
...
async def do_mailing(update, context):
    ...
    uid = query.from_user.id
    state = get_state(uid)
    log.info(f"Broadcast: do_mailing Triggered By Admin {uid}, In-Memory State: {state}")

    # Try In-Memory State First, Then Fall Back To Database
    src_chat = None
    src_msg = None

    if state and "msg_chat_id" in state.get("data", {}):
        src_chat = state["data"]["msg_chat_id"]
        src_msg = state["data"]["msg_id"]
        log.info(f"Broadcast: Using In-Memory State chat={src_chat} msg={src_msg}")
    else:
        # Fall Back To Database
        db_chat = db.get_setting(f"_broadcast_chat_{uid}")
        db_msg = db.get_setting(f"_broadcast_msg_{uid}")
        log.info(f"Broadcast: In-Memory State Missing, Trying DB: chat={db_chat} msg={db_msg}")
        if db_chat and db_msg:
            try:
                src_chat = int(db_chat)
                src_msg = int(db_msg)
                log.info(f"Broadcast: Recovered From DB chat={src_chat} msg={src_msg}")
            except (ValueError, TypeError) as e:
                log.error(f"Broadcast: Could Not Parse DB State: {e}")

    if src_chat is None or src_msg is None:
        log.warning(f"Broadcast: No Captured Message Found For Admin {uid}")
        return await safe_edit(
            query,
            "❌ No Message To Send.\n\n"
            "Please Open 🔆 General Functions → 📢 Mail Again, "
            "Then Send Your Post Before Pressing ✅ Send.",
        )

    # Clear State And DB Markers Now That We Have The Info
    clear_state(uid)
    db.set_setting(f"_broadcast_chat_{uid}", "")
    db.set_setting(f"_broadcast_msg_{uid}", "")

    all_ids = db.get_all_user_ids()
    total = len(all_ids)
    if total == 0:
        return await safe_edit(query, "❌ No Users To Send To.")
    ...
    try:
        ...
        await progress_msg.edit_text(final)
