...
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state:
        ...
        if action == "mail_wait_content":
            # Save The State FIRST (Both In-Memory AND In Database For Persistence)
            set_state(uid, "mail_confirm", msg_chat_id=msg.chat_id, msg_id=msg.message_id)
            # ALSO Save To Database So It Survives Any State Issue
            db.set_setting(f"_broadcast_chat_{uid}", str(msg.chat_id))
            db.set_setting(f"_broadcast_msg_{uid}", str(msg.message_id))
            log.info(f"Broadcast: Captured Message id={msg.message_id} chat={msg.chat_id} from admin={uid} (Saved To DB)")

            # Get User Count
            try:
                user_count = db.get_user_count()
            except Exception as e:
                log.error(f"Broadcast: get_user_count Failed: {e}")
                user_count = 0

            # Echo The Post Back To Admin (Preview)
            try:
                ...
                log.error(f"Broadcast: copy_message Preview Failed: {e}")

            # Send Confirmation Prompt With Send/Cancel Buttons
            # Use Plain Text (No Markdown) To Avoid Any Parse Errors
            try:
                ...
                log.error(f"Broadcast: Confirmation reply_text Failed: {e}")
                # Fallback: Try Without Reply Markup
                try:
                    await msg.reply_text(
                        f"📢 Send This Post To {user_count} Users?\n"
                        f"Reply /mailsend To Send Or /mailcancel To Cancel."
                    )
                except Exception as e2:
                    log.error(f"Broadcast: Fallback Confirmation Also Failed: {e2}")
            return
